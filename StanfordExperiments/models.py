...
def vgg_19(conv_blocks, width, height, channels, num_classes, weight_decay, efficient = False, dataAug = False, augmentation = False, **kwargs):
...
def resNet50(input_shape, num_of_output_classes, augmentation = False, **kwargs):
...
class DeepResidual_Block(tf.keras.layers.Layer):
    def __init__(self, filters, strides=1, activation="relu", **kwargs):
        super().__init__(**kwargs)
        self.activation = tf.keras.activations.get(activation)
        ...
        self.main_layers = [
            ...
            self.activation,
            # The stride of the second Conv layer is always 1
            tf.keras.layers.Conv2D(filters*4, kernel_size=1, strides=1, padding="SAME", use_bias=False),
            tf.keras.layers.BatchNormalization()]
        self.skip_layers = []
        # If the stride of the first Conv layer is 2,
        #    then add a 1 x 1 Conv layer on the skip channel, followed by batch normalization
...
def DeepResNet50(input_shape, num_classes, augmentation = False, **kwargs):
    
    resnet = tf.keras.models.Sequential(name='DeepResNet-50')
    ...
    for filters in [64] * 6 + [128] * 6 + [256] * 6 + [512] * 6:
        strides = 1 if filters == prev_filters else 2
        resnet.add(DeepResidual_Block(filters, strides=strides))
        prev_filters = filters
