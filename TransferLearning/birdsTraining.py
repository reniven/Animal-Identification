...
(train_ds, test_ds) , info_train = tfds.load('caltech_birds2010', split=['train', 'test'], with_info=True,  as_supervised=True)
...
with tf.device('/cpu:0'):
    train_loader = prepare_dataset(train_ds, size_of_mini_batch, 300, shuffle=True, augment=False, buffer_size=buffer_size, hotEncode=True, num_classes=200)
    test_loader = prepare_dataset(test_ds, size_of_mini_batch_test, 300, hotEncode=True, num_classes=200)
...
with strategy.scope():
    ...
    model = resNet50((IMG_SIZE, IMG_SIZE, 3), 200,  augmentation = True,
                        resize = True,
                        increased_size = 250,
                        random_crop = True,
                        original_size = 200,
...
if(savedModelUse == True):
    ...
    savedModel = tf.keras.models.load_model('/lustre/work/netthinker/ele3/Research/TransferLearning/birdModel')
